...
class MainWindow(QWidget):
    def __init__(self, *args, **kwargs):
        ...
        layout.addWidget(QLabel("Season Year:"), 0, 0)
        self.season_year = QLineEdit(self)
        layout.addWidget(self.season_year, 0, 1)
        layout.addWidget(QLabel("Event Code:"), 1, 0)
        self.event_code = QLineEdit(self)
        layout.addWidget(self.event_code, 1, 1)
        layout.addWidget(QLabel("Event Name:"), 2, 0)
        ...
        self.button_FMS.clicked.connect(
            lambda: self.handleFMS(
                self.season_year.text(), self.event_code.text().upper(), self.textFMS
            )
        )
        ...
        layout.addRow(
            QLabel(
                "<b>Connect the YouTube channel you want to upload to, accepting both permissions (read & write videos)</b>"
            )
        )
        # Authentication
        self.textYouTube = QLabel(
            '<font color="red">YouTube not yet authenticated</font>'
        )
        ...
        layout.addRow(
            QLabel(
                "<i>program will automatically add year, event code, and program (FRC/FTC)</i>"
            )
        )
        ...
        layout.addRow(
            QLabel(
                "<b>Choose a sponsor logo OR fill out more event info for YouTube thumbnail</b>"
            )
        )
        # Sponsor Image
        self.img_EventSponsor = QPushButton("Select File")
        self.img_EventSponsor.clicked.connect(
            lambda: self.getFileSponsorImage(self.img_EventSponsor)
        )
        layout.addRow("Sponsor Logo:", self.img_EventSponsor)
        # Verbose event details
        layout.addRow(QLabel("⸻ or ⸻"))
        self.eventBuilding = QLineEdit(self)
        layout.addRow("Building:", self.eventBuilding)
        self.eventCity = QLineEdit(self)
        layout.addRow("City:", self.eventCity)
        self.eventDates = QLineEdit(self)
        layout.addRow("Dates:", self.eventDates)
        ...
        self.image_thumbnail = QLabel(
            '<font color="red">Generate a test thumbnail</font>'
        )
        self.button_thumbnail = QPushButton("Test Thumbnail")
        self.button_thumbnail.clicked.connect(
            lambda: self.handleThumbnail(
                [
                    self.eventBuilding.text(),
                    self.eventCity.text(),
                    self.eventDates.text(),
                ],
                self.image_thumbnail,
                self.thumbnail_force.isChecked(),
            )
        )
        ...
        layout.addRow(
            QLabel(
                "<b>Define how to chop up the livestream into matches</b><br><i>Units are in seconds, decimals allowed.</i>"
            )
        )
        # 2024 values: [3, 155, 5, -7.25, 16.67]
        self.season_secondsBefore = QLineEdit(str(3 + 3.159))
        layout.addRow("Before Match :", self.season_secondsBefore)
        self.season_matchDuration = QLineEdit(str(15 + 5 + 135))
        layout.addRow("Match Duration :", self.season_matchDuration)
        self.season_secondsAfterEnd = QLineEdit(str(5 + 3))
        layout.addRow("After Match :", self.season_secondsAfterEnd)
        self.season_secondsBeforePost = QLineEdit("-8.06")
        layout.addRow("Before Post :", self.season_secondsBeforePost)
        self.season_secondsAfterPost = QLineEdit(str(25 + 8))
        layout.addRow("After Post :", self.season_secondsAfterPost)
        layout.addRow(
            QLabel("<i>These should add to 179.94 to get a 3:00 video on YouTube</i>")
        )
        ...
        self.twitchUser = QLineEdit("firstinrobotics")
        layout.addRow("Twitch User:", self.twitchUser)
        ...
        self.TBA_eventCode = QLineEdit()
        layout.addRow(QLabel("TBA event code:"), self.TBA_eventCode)
        self.TBA_AuthID = QLineEdit()
        layout.addRow(QLabel("Associated Auth ID:"), self.TBA_AuthID)
        self.TBA_AuthSecret = QLineEdit()
        layout.addRow(QLabel("Associated Auth Secret:"), self.TBA_AuthSecret)

        # FMS pull button
        self.button_TBA = QPushButton("Verify TBA")
        self.button_TBA.setStyleSheet("color: red")
        self.button_TBA.clicked.connect(
            lambda: self.handleTBA(
                self.TBA_AuthID.text(),
                self.TBA_AuthSecret.text(),
                self.TBA_eventCode.text(),
            )
        )
    ...
    def handleFMS(self, year, eventCode, text):
        ...
        try:
            if self.program.currentText() == "FRC":
                matchesRaw = getMatchesFromFMS(
                    year,
                    eventCode,
                    self.program.currentText(),
                    CREDENTIALS["FRC_username"],
                    CREDENTIALS["FRC_key"],
                )
            ...
            if len(self.matches) > 0:
                ...
                text.setText(
                    ...
                    + " matches found for "
                    + eventCode
                    + ". If event has begun, verify event code.</font>"
                )
    ...
    def test_twitch(self):
        ...
        try:
            self.twitchUserID = covertID2Username(
                CREDENTIALS["Twitch_clientID"],
                CREDENTIALS["Twitch_clientSecret"],
                self.twitchUser.text(),
            )
    ...
    def handleYouTube(self):
        self.textYouTube.setText(
            '<font color="aqua">Authenticate using browser...</font>'
        )
    ...
    def handleThumbnail(self, data, image, forceText=False):
        ...
        matchInfo = {
            "id": matchType + str(matchNum),
            "start": datetime.datetime.now(),
            "post": datetime.datetime.now() + datetime.timedelta(seconds=155),
            "teamsRed": teams[0:3],
            "teamsBlue": teams[3:6],
        }
        ...
        if forceText:
            generateThumbnail(
                matchInfo, programImagePath, eventDetails, None, "./images/trial"
            )
        ...
        try:
            ...
            CONFIG = {
                ...
                "season": {
                    "year": (
                        -1
                        if self.season_year.text() == ""
                        else int(self.season_year.text())
                    ),
                    "secondsBeforeStart": float(self.season_secondsBefore.text()),
                    "secondsOfMatch": float(
                        self.season_matchDuration.text()
                    ),  # auto + bell (~5) + teleop
            ...
            if self.twitchUserID == None:
                CONFIG["video"] = {
                    ...
                    "matchID": self.match_type.currentText()[0]
                    + self.match_number_ref.text(),
                ...
                CONFIG["video"] = {
                    ...
                    "streamDelay": float(self.twitchDelay.text()),
                }
    ...
    def loadCONFIG(self, button):
        ...
        if response[0] != "":
            ...
            with open(response[0], "r") as file:
                ...
                self.season_secondsBefore.setText(
                    str(CONFIG["season"]["secondsBeforeStart"])
                )
                self.season_matchDuration.setText(
                    str(CONFIG["season"]["secondsOfMatch"])
                )
                self.season_secondsAfterEnd.setText(
                    str(CONFIG["season"]["secondsAfterEnd"])
                )
                self.season_secondsBeforePost.setText(
                    str(CONFIG["season"]["secondsBeforePost"])
                )
                self.season_secondsAfterPost.setText(
                    str(CONFIG["season"]["secondsAfterPost"])
                )

                self.video_description.setPlainText(CONFIG["YouTube"]["description"])
                self.video_tags.setText(CONFIG["YouTube"]["tags"])
                self.video_playlist.setText(
                ...
                if CONFIG["video"]["type"] == "static":
                    ...
                    self.match_type.setCurrentText(
                        {"Q": "Q = Quals", "P": "P = Playoffs", "F": "F = Finals"}[
                            CONFIG["video"]["matchID"][0]
                        ]
                    )
